...
object ClassUtils {
    fun findClasses(
        ...
        val rootPath = root.toString().removeSuffix("!/$packagePath")
        val rootURI = URI(rootPath)
        val isJar = rootPath.startsWith("jar")

        val classes = ArrayList<Class<*>>()

        if (isJar) {
            val file = Paths.get(URI(rootURI.schemeSpecificPart)).toFile()
